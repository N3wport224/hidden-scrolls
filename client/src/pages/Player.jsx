...
export default function Player() {
  ...
  // Use a manual click to force the audio engine to "wake up"
  const startPlayback = () => {
    if (audioRef.current) {
      audioRef.current.load(); // Forces fresh header request
      const savedTime = localStorage.getItem(`progress_${id}`);
      if (savedTime) audioRef.current.currentTime = parseFloat(savedTime);
      audioRef.current.play().catch(e => console.log("Unlock required..."));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center p-6">
      <div className="w-full max-w-3xl flex justify-between mb-8">
        <button onClick={() => navigate('/')} className="font-bold">← Library</button>
      </div>
      ...
      <div className="w-full max-w-3xl bg-slate-800 p-8 rounded-2xl shadow-lg mb-8 text-center">
          <button 
            onClick={startPlayback}
            className="mb-6 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 px-10 rounded-full shadow-xl"
          >
            START AUDIO ENGINE
          </button>
      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="bg-slate-800 rounded-xl divide-y divide-slate-700 max-h-64 overflow-y-auto">
          {book.media?.chapters?.map((c, i) => (
            <button key={i} onClick={() => { if(audioRef.current) { audioRef.current.currentTime = c.start; audioRef.current.play(); } }} className="w-full p-4 hover:bg-slate-700 flex justify-between text-left">
