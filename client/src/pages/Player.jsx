...
export default function Player() {
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      try {
        console.log("🛠 Authorizing isolated car-player session...");
        const res = await fetch(getProxyUrl(`/api/items/${id}/play`), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          // CRITICAL: Isolated credentials to avoid session conflicts
          credentials: 'same-origin', 
          body: JSON.stringify({ 
            deviceId: 'hidden-scrolls-pi-car-v2', // New unique ID
      ...
      <div className="w-full max-w-3xl flex flex-col items-center">
        ...
        <div className="w-full bg-slate-800 p-6 rounded-xl shadow-lg mb-8 text-center">
            <audio 
              ref={audioRef} 
              controls 
              // Remount ONLY when authorized session is ready
