...
export default function Player() {
  ...
  const [sessionId, setSessionId] = useState(null); // Captured from handshake
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      try {
        ...
        // Audiobookshelf returns the session ID in the 'id' field
        if (data.id) {
  ...
  // This format matches the official ABS web client
      ...
      <div className="w-full max-w-3xl flex flex-col items-center relative">
        ...
        <div className="w-full bg-slate-800 p-6 rounded-xl shadow-lg mb-8">
            ...
            <audio 
              ref={audioRef} 
              controls 
              key={sessionId} // Forces the browser to reload the audio element once the ID is ready
